...
if __name__=="__main__":
    data_path = "/projects/MAD3D/Zhuoli/MICCAI/VerSe2019/dataset-verse19test"
    json_path = os.path.join(data_path, "derivatives")
    json_all = os.listdir(json_path)
    json_all.sort()
    ct_base_path = os.path.join(data_path, "rawdata")
    ct_list = os.listdir(ct_base_path)
    ct_list.sort()
    drr_path = os.path.join(data_path, "enhance_drr/")
    ...
    for n, drr_folder in enumerate(drr_folders):
        ...
        ct_path_folder = os.path.join(ct_base_path,ct_list[n])
        ct_files = os.listdir(ct_path_folder)

        for ct_file in ct_files:
            ct_path = os.path.join(ct_path_folder, ct_file)
            ...
            # 3
            json_file = get_json(os.path.join(json_path,json_all[n], ct_file.replace('_ct.nii.gz', '_seg-vb_ctd.json')))
            centroids = []
            ids = [] 
            ...
            for m,centroids_drr in enumerate(centroids_drr_all):
                ...
                target_base_path = heatmap_path
                os.makedirs(target_base_path, exist_ok=True)
                txt_name = os.path.join(target_base_path, drr_folder+ "_" + str(m) + ".txt")
                txt_file = open(txt_name, 'w')
                for c in centroids_drr:
